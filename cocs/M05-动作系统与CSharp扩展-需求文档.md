...
| parameterMap | Dictionary<string, object> | 可选 | 传入动作的参数表 | 空字典表示无参数 | 空字典 |
...
- 状态变化：`Lookup action -> Create action instance -> Bind ActionContext -> Execute coroutine -> Return success/failure`。
- 数据提交时机：动作参数在执行前一次性绑定；执行结果在协程结束或抛异常时提交给 `StepResult`。
- 取消/回退：运行中止时，动作执行器应在下一次 `yield` 返回后感知取消标记并停止；动作本身不提供通用回滚。
- 自定义动作发现：V1 支持特性扫描与显式注册两种方式；发生名称冲突时，显式注册优先，且必须报 `ACTION_NAME_CONFLICT`。
- 参数映射：所有 YAML 字段统一映射到 `Dictionary<string, object>`；动作实现者自行读取并做类型转换。
...
| 内置动作参数必须满足契约 | 执行动作前 | Error | `步骤 {stepName} 缺少参数 {parameter}` |
...
- 关键类与职责：
  - `IAction`：统一动作执行契约。
  - `ActionNameAttribute` `(设计提案，实现时确认)`：为自定义动作声明 YAML 动作名。
  - `ActionRegistry` `(设计提案，实现时确认)`：管理动作注册、解析与实例创建。
  - `ActionContext` `(设计提案，实现时确认)`：向动作提供 Finder、Options、Reporter、Simulator、SharedBag。
- 核心流程：
```text
Initialize ActionRegistry
-> Register built-in actions
-> Scan custom actions
-> On step execution, resolve action by name
-> Build ActionContext
-> action.Execute(root, context, parameters)
-> Convert coroutine completion to StepResult
```
- 性能约束：
...
- TODO(待确认)：官方 `PanelSimulator` 在目标包版本下可直接支撑的动作集合、方法签名与键盘输入限制需原型验证。
...
- 空数据：不需要参数的动作在 `parameterMap` 为空字典时仍必须能执行。
- 单元素：对单个目标元素的 `click`、`hover`、`assert_visible` 必须在找到第一个元素后立即执行，不再继续查找其他候选。
- 上下限临界值：动作名长度 `1` 与 `64` 合法；超界注册时报错。
- 异常数据恢复：动作执行异常后不得污染注册表；后续步骤仍可解析其他动作实例。

## 11. 周边可选功能

- P1：支持动作级参数 schema 声明；当前预留 `IActionDescriptor` 扩展接口。
- P1：支持 DI 容器创建自定义动作；当前预留 `IActionFactory` 扩展点。

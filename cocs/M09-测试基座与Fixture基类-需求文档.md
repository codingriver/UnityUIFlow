...
| --- | --- | --- | --- | --- |
| 初始化测试基座 | NUnit `[SetUp]` 生命周期 | `TWindow` 无法创建；窗口初始化失败 | `UnityUIFlowFixture<TWindow>.SetUp` `(设计提案，实现时确认)` | 不涉及；每次测试前重新初始化 |
| 获取窗口根节点 | 测试代码访问 `window.rootVisualElement` 或封装属性 | 窗口未创建；窗口已销毁 | `UnityUIFlowFixture<TWindow>.Root` `(设计提案，实现时确认)` | 不涉及；只读操作 |
...
| 清理窗口与上下文 | NUnit `[TearDown]` 生命周期 | 当前未创建窗口 | `UnityUIFlowFixture<TWindow>.TearDown` `(设计提案，实现时确认)` | 不涉及；销毁测试现场，不支持恢复 |
...
- 扩展入口：派生类允许覆写 `SetUp`、`TearDown`、`CreateDefaultOptions`，但必须调用基类实现。
...
| 规则 | 检查时机 | 级别 | 提示文案 |
| --- | --- | --- | --- |
| `TWindow` 必须继承 `EditorWindow` | 创建测试基座前 | Error | `测试窗口类型必须继承 EditorWindow` |
...
| 测试清理失败 | FIXTURE_TEARDOWN_FAILED | `测试清理失败：{detail}` | 记录错误并继续 NUnit 清理流程 |
...
- 关键类与职责：
  - `UnityUIFlowFixture<TWindow>` `(设计提案，实现时确认)`：统一测试基类。
  - `EditorWindowUITestFixture<TWindow>`：底层生命周期宿主，由 Unity 官方测试框架提供。
  - `FixtureExecutionBridge` `(设计提案，实现时确认)`：把基座状态转换为执行引擎可消费的 `ExecutionContext`。
- 核心流程：
```text
NUnit SetUp
-> Create or open TWindow
-> Validate rootVisualElement
...
- TODO(待确认)：`EditorWindowUITestFixture<TWindow>` 在目标 Unity 版本和目标 UI Test Framework 版本下的具体生命周期钩子与窗口创建方式需原型验证。

## 9. 验收标准
1. [定义继承 `UnityUIFlowFixture<MyWindow>` 的测试类] -> [执行测试] -> [`SetUp` 后可访问有效的 `Window`、`Root`、`Finder`、`Screenshot`]
...
- 上下限临界值：单测试内只能存在 1 个当前宿主窗口；重复创建同类型窗口时必须复用或显式关闭旧实例。
- 异常数据恢复：若 `TearDown` 失败，不得影响后续测试继续启动；必须记录错误并尽可能释放资源。

## 11. 周边可选功能

- P1：支持基座级测试数据注入；当前预留 `CreateExecutionContext` 扩展点。
- P1：支持多窗口测试宿主；当前不进入 V1，仅预留 `AdditionalWindows` 扩展集合。

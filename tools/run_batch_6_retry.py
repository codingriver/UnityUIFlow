#!/usr/bin/env python3
"""Retry batch 6 after fixing AssertionFailed -> Failed mapping."""
...
def main() -> int:
    ...
    batch_files = all_files[60:70]  # batch 6
    batch_report_dir = f"{REPORT_DIR}/batch_006"
    ...
    print(f"\n[Batch 6] Running {len(batch_files)} file(s)...")
    ...
    if not parsed["ok"]:
        print(f"[Batch 6] MCP call failed: {parsed.get('error')}")
        return 1
    ...
    print(
        f"[Batch 6] Status={parsed['status']} total={parsed['total']} "

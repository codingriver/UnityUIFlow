...
│   ├── Headed/                  ← Editor 可视化窗口（BatchRunner、HeadedTestWindow）
...
- **Headed 单条执行**：通过 `UnityUIFlow > UnityUIFlow` Editor 窗口加载 YAML 并运行。
- **Headed 批量执行**：`UnityUIFlow > Batch Runner`，选择 YAML 目录运行。

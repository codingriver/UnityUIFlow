...
## M27: 编辑器窗口 / UIToolkit / E2E 扩展

**前置:** 与 M26 相同；默认布局需包含 **Game** 视图（T-M27-03）；编辑器 UI 语言建议 **英文**（窗口标题 `Inspector` / `Game`）。

以下用例由 `scripts/run_acceptance_suite.py` 以 **T-M27-01** … **T-M27-08** 顺序执行（需 Unity 已连接）。

### T-M27-01: editor.windows.list — closable 字段
**预期:** 每个窗口项含 `closable`、`closeDeniedReason`。

### T-M27-02: editor.window.close — 未找到
**调用:** `unity_editor_window_close(windowTitle="__m27_missing_window__")`  
**预期:** 错误码 `WINDOW_NOT_FOUND`。

### T-M27-03: editor.window.close — 黑名单
**调用:** `unity_editor_window_close(windowTitle="Game")`（Game 视图打开时）  
**预期:** `WINDOW_CLOSE_DENIED`。

### T-M27-04: uitoolkit.event — wheel
**预期:** 对带 `name` 的 `Label` 派发 `wheel` 成功（`wheelDeltaY` 非零）。

### T-M27-05: editor.window.setRect — 停靠窗口
**调用:** `unity_editor_window_set_rect` 匹配标题含 `Inspector`  
**预期:** `WINDOW_DOCKED`（默认停靠布局）。

### T-M27-06: uitoolkit.scroll — 嵌套路径失败
**调用:** `scrollViewNamePath` 为不存在的 `outer|inner`  
**预期:** 结果 payload `ok=false`，`state` 含 `SCROLLVIEW_NOT_FOUND`。

### T-M27-07: editor_e2e_run — exportZip
**步骤:** 对 `e2e-specs/examples/smoke_editor_state.yaml` 执行 `exportZip=true`  
**预期:** 返回 `zipPath` 且磁盘存在 `e2e-bundle.zip`。

### T-M27-08: capturePointer / releasePointer
**预期:** 对命名控件依次 `capturePointer`、`releasePointer` 均 `ok=true`。

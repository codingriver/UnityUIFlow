...
2. **安装 Unity 插件（UPM）**：在目标 Unity 工程的 `Packages/manifest.json` 中加入依赖（与下方「Unity 插件（UPM）」一节相同），打开编辑器并保持工程可编译。
3. **运行服务**：
   ```bash
...
> 临时限制：`unitypilot` 当前不开放 `-batchmode` 用法。请使用正常的 Unity Editor 模式连接 MCP。

## Unity 插件（UPM）

Unity 侧为同一 Git 仓库中的 UPM 包，路径为 `unitypilot-editor/`。**远程安装**（任意工程）在 `Packages/manifest.json` 中写入：
...
需要锁定版本时，在 URL 后加标签，例如：`...unitypilot.git?path=/unitypilot-editor#v0.1.0`（请使用仓库中已发布的 tag）。

**在本仓库内开发**时，`UnityPilot/` 示例工程使用本地路径引用，与发布用 Git URL 指向**同一套文件**：

```json
"io.github.codingriver.unitypilot-editor": "file:../../unitypilot-editor"
```

（路径相对于 `UnityPilot/Packages/manifest.json`。）
...
适合 Claude Desktop、Cursor、Windsurf、OpenCode 这类以本地命令启动 MCP Server 的客户端。

```bash
python run_unitypilot_mcp.py --transport stdio --port 8765
```

### Streamable HTTP（本机单用户）

适合支持 remote MCP / Streamable HTTP 的客户端或本地调试场景。

```bash
python run_unitypilot_mcp.py --transport http --port 8765 --http-port 8000
```

或：

```bash
unitypilot-mcp --transport http --port 8765 --http-port 8000
```

说明：

- `--port` 仍是 Python 与 Unity Editor Bridge 之间的内部 WebSocket 端口。
- `--http-port` 是 MCP Streamable HTTP 对外监听端口。
- 默认仅监听 `127.0.0.1`，当前方案面向本机单用户，不建议直接暴露到公网。
- FastMCP 的默认 Streamable HTTP 路径为 `/mcp`。
...
将仓库中的 `mcp.example.json` 复制为项目根目录的 `.mcp.json`，或使用 Cursor 时在项目下创建 `.cursor/mcp.json`（把示例中的 `<PATH_TO_UNITYPILOT_REPO>` 换成本机绝对路径）。

#### 方案 A：stdio / local command（默认，适合 Cursor）
```json
{
  "mcpServers": {
    "unitypilot": {
...
#### 方案 B：Streamable HTTP / remote MCP
先手动启动服务：

```bash
python D:/unitypilot/run_unitypilot_mcp.py --transport http --port 8765 --http-port 8000 --log-file D:/unitypilot/logs/http-mcp.log
```

然后在支持 remote MCP / Streamable HTTP 的客户端中配置远程地址：

```text
http://127.0.0.1:8000/mcp
```

> 说明：Cursor 这类 `local command` 配置通常仍然使用 stdio；HTTP 模式更适合支持远程 MCP 地址的客户端或联调场景。
...
- `.cursor/mcp.json` 中保留 `--transport stdio`
- `--port` 配成和 Unity Editor Bridge 一致

### 如果你要验证 Streamable HTTP
使用下面两步：

1. 单独启动 HTTP 服务：

```bash
python run_unitypilot_mcp.py --transport http --port 8765 --http-port 8000 --log-file D:/unitypilot/http.log
```

2. 在支持 remote MCP 的客户端中连接：
...
|---|---|---|---|---|
| 传输模式 | `--transport` | `UNITYPILOT_TRANSPORT` | `stdio` | MCP 传输模式：`stdio` 或 `http` |
| Unity WS 主机 | `--host` | `UNITYPILOT_HOST` | `127.0.0.1` | Python 与 Unity Editor Bridge 的 WebSocket 监听地址 |
| Unity WS 端口 | `--port` | `UNITYPILOT_PORT` | `8765` | Python 与 Unity Editor Bridge 的 WebSocket 监听端口 |
| MCP HTTP 主机 | `--http-host` | `UNITYPILOT_HTTP_HOST` | `127.0.0.1` | Streamable HTTP 监听地址，仅 `--transport http` 时使用 |
| MCP HTTP 端口 | `--http-port` | `UNITYPILOT_HTTP_PORT` | `8000` | Streamable HTTP 监听端口，仅 `--transport http` 时使用 |
...
- 始终检查 Unity WebSocket 端口。
- 当 `--transport http` 时，会额外检查 HTTP 监听端口。
- 如果端口被占用，服务会尝试重新检查 5 次，每次间隔 5 秒。如果全部失败，程序将自动退出。
...
```bash
python run_unitypilot_mcp.py --transport http --port 8765 --http-port 8000 --log-file D:/unitypilot/mcp.log
```

验证点：

1. 日志中出现 `transport=http` 和 HTTP 监听地址。
2. Unity Editor 仍连接 `--port` 指定的 websocket 端口。
3. 支持 Streamable HTTP 的客户端使用 `http://127.0.0.1:8000/mcp` 连接。
4. `--log-file` 中能看到既有的 websocket/tool 调用日志。

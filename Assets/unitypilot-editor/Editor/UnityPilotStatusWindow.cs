...
namespace codingriver.unity.pilot
{
    internal class UnityPilotStatusWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            UnityPilotWindowDiagnostics.RecordWindow(position.width, position.height, 0);
        ...
        private void DrawSharedEndpointSection(UnityPilotBridge bridge, BridgeStatus status)
        {
            ...
            using (new EditorGUILayout.VerticalScope(_styleBox))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    mgr.HttpPort = EditorGUILayout.IntField(mgr.HttpPort, GUILayout.Width(60));
                ...
                using (new EditorGUI.DisabledScope(status.IsStarted))
                {
                    if (GUILayout.Button("应用连接地址", GUILayout.Height(22)))
                    {
                        ...
                        mgr.WsPort = _wsPortInput;
                        ShowToast($"已应用 ws://{_wsHostInput}:{_wsPortInput}  http://{_wsHostInput}:{mgr.HttpPort}/mcp");
                    }
                }
                ...
                // Port consistency warning
                if (bridge.WsPort != mgr.WsPort)
                {
                    EditorGUILayout.HelpBox(
                        $"⚠ 端口不一致：Bridge 连接 ws://{bridge.WsHost}:{bridge.WsPort}，但 MCP 服务器 WS 端口是 {mgr.WsPort}。" +
                        "请停止 Bridge 后点击【应用连接地址】同步。",
                        MessageType.Warning);
                }
            }
        }
        ...
        private void DrawMcpServerSection(BridgeStatus status)
        {
            ...
            using (new EditorGUILayout.VerticalScope(_styleBox))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        st = $"运行中  PID={mcpStatus.ProcessId}  HTTP={(mcpStatus.HttpPortListening ? "监听" : "未监听")}  WS={(mcpStatus.WsPortListening ? "监听" : "未监听")}";
                    }
                    else
                    {
                        st = "已停止";
        ...
        private static string BuildServerTestResult(BridgeStatus status, string wsHost, int wsPort)
        {
            ...
            if (!status.IsWsOpen)
            {
                ...
                {
                    sb.AppendLine($"  MCP WS端口 {mgr.WsPort} 无法连通 → 服务器未监听该端口，请检查端口配置一致性");
                }
                if (wsPort != mgr.WsPort)
                {
                    sb.AppendLine($"  ⚠ 端口不一致！Bridge连接 {wsPort}，MCP服务器监听 {mgr.WsPort}");
                }

...
namespace UnityUIFlow
{
    ...
    internal static class ImguiBridgeRegistry
    {
        ...
        internal static void InstallOnGuiHook(EditorWindow window, ImguiExecutionBridge bridge)
        {
            ...
            if (onguiMethod == null)
            {
                Debug.LogWarning($"[UnityUIFlow] Could not find OnGUI method on {windowType.Name}");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log($"[UnityUIFlow] MonoHook installed on {windowType.Name}.OnGUI");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[UnityUIFlow] Failed to install MonoHook on {windowType.Name}.OnGUI: {ex.Message}");
            }
        }
        ...
        internal static void UninstallOnGuiHook(EditorWindow window)
        {
            ...
            if (!hasOtherWindowsOfType && Hooks.TryGetValue(windowType, out var hook))
            {
                try
                {
                    ...
                    Debug.Log($"[UnityUIFlow] MonoHook uninstalled from {windowType.Name}.OnGUI");
                }
                catch (Exception ex)
                {
                    Debug.LogWarning($"[UnityUIFlow] Failed to uninstall MonoHook: {ex.Message}");
                }
            }
        }
        ...
        private static void OnGUIProxy(EditorWindow __this)
        {
            ...
            Debug.LogError("[UnityUIFlow] OnGUIProxy should never be called directly!");
        }
        ...
        internal static void InvokeOriginalOnGUI(EditorWindow window)
        {
            ...
            try
            {
                Debug.Log($"[UnityUIFlow] InvokeOriginalOnGUI calling OnGUIProxy for {window.GetType().Name}, Event.current={Event.current?.type}");
                OnGUIProxy(window);
                Debug.Log($"[UnityUIFlow] InvokeOriginalOnGUI OnGUIProxy returned for {window.GetType().Name}");
            }
            catch (Exception ex) when (ex.GetType().Name.Contains("ExitGUIException") || ex.GetType().Name.Contains("TargetInvocationException"))
            {
                Debug.Log($"[UnityUIFlow] InvokeOriginalOnGUI caught {ex.GetType().Name} for {window.GetType().Name}");
            }
        }
        ...
        private static void InstallGetRectHook()
        {
            ...
            if (getRectMethod != null)
            {
                try
                {
                    ...
                    Debug.Log("[UnityUIFlow] MonoHook installed on GUILayoutUtility.GetRect");
                    return;
                }
                catch (Exception ex)
                {
                    Debug.LogWarning($"[UnityUIFlow] Failed to install MonoHook on GUILayoutUtility.GetRect: {ex.Message}");
                }
            }
            ...
            {
                Debug.LogWarning("[UnityUIFlow] Could not find GUILayoutUtility.DoGetRect for hooking.");
                return;
            }
            ...
            try
            {
                ...
                _getRectHook.Install();
                Debug.Log("[UnityUIFlow] MonoHook installed on GUILayoutUtility.DoGetRect");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[UnityUIFlow] Failed to install MonoHook on GUILayoutUtility.DoGetRect: {ex.Message}");
            }
        }
        ...
        private static Rect DoGetRectReplacement(GUIContent content, GUIStyle style, GUILayoutOption[] options)
        {
            ...
            if (_currentOnGUIWindow != null)
            {
                ...
                UnityEngine.Debug.Log($"[UnityUIFlow] DoGetRectReplacement: style={styleName}, content={content?.text}, rect={result}");
        ...
        private static void InstallEditorPopupHook()
        {
            ...
            if (popupMethod == null)
            {
                Debug.LogWarning("[UnityUIFlow] Could not find EditorGUILayout.Popup for hooking.");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log("[UnityUIFlow] MonoHook installed on EditorGUILayout.Popup");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[UnityUIFlow] Failed to install MonoHook on EditorGUILayout.Popup: {ex.Message}");
            }
        }
        ...
        private static void InstallGuiStyleDrawHook()
        {
            ...
            if (drawMethod == null)
            {
                Debug.LogWarning("[UnityUIFlow] Could not find GUIStyle.Draw for hooking.");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log("[UnityUIFlow] MonoHook installed on GUIStyle.Draw");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[UnityUIFlow] Failed to install MonoHook on GUIStyle.Draw: {ex.Message}");
            }
        }
        ...
        {
            Debug.LogError("[UnityUIFlow] DrawProxy should never be called directly!");
        }
